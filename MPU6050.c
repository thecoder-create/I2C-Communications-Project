...
void MPU6050_Get_Angle(MPU6050_ACCEL_t* Accel_Instance, MPU6050_GYRO_t* Gyro_Instance, MPU6050_ANGLE_t* Angle_Instance){
    ...
    Accel_Angle_X = atan2(Accel_Instance->Ay, sqrt(pow(Accel_Instance->Ax, 2) + pow(Accel_Instance->Az, 2)));
    Accel_Angle_Y = atan2(-Accel_Instance->Ax, sqrt(pow(Accel_Instance->Ay, 2) + pow(Accel_Instance->Az, 2)));

    // Convert to degrees
    Accel_Angle_X *= RAD_TO_DEGREE_CONV;
    Accel_Angle_Y *= RAD_TO_DEGREE_CONV;

    // Gyroscope angles are already in degrees
    Angle_Instance->ArX = Accel_Angle_X;
    Angle_Instance->ArY = Accel_Angle_Y;

    // Gyroscope Z angle is just cumulative angle from gyroscope data
    Angle_Instance->ArZ += Gyro_Instance->Gz / 131.0; // 131.0 is the sensitivity scale factor for the gyroscope

    // Ensure angle stays within -180 to 180 degrees
    if (Angle_Instance->ArZ > 180) {
        Angle_Instance->ArZ -= 360;
    } else if (Angle_Instance->ArZ < -180) {
        Angle_Instance->ArZ += 360;
    }
